...
// Simplifico lineas de codigo quitando espacios y return

app.get('/api/productos', (req, res) => {
    if (productosArray.length < 1) { res.status(404).json({ error: 'Producto no encontrado', }); }
    res.json(productosArray);
});
...
const server = app.listen(PORT, () => {
    try {console.log(`Servidor conectado en puerto: ${PORT}`)} 
    catch {error = console.log('Error: ' + error.toString())} --> Aca podemos usar templete strings por ejemplo 
}); 
​
*/


// Quizas esta es la manera mas optima de hacer intercetar errores
